"""
FINAL CEILING-BREAKING MODEL - Insurance Agent NILL Prediction
Target: >95% Accuracy

Key approach:
1. Keep the exact core feature engineering from the 0.946 model
2. Highly calibrated threshold with false positive reduction
3. Special handling for agents with fluctuating patterns
4. Two-stage prediction with correction for borderline cases
"""
import os
import pandas as pd
import numpy as np
...
print("=" * 100)
print("FINAL CEILING-BREAKING MODEL - NILL PREDICTION")
print("=" * 100)
...
for df in [train_df, test_df]:
    ...
    # NEW: Pattern detection features for fluctuating agents
    if all(col in df.columns for col in ['unique_proposal', 'unique_quotations', 'unique_proposals_last_7_days', 'unique_quotations_last_7_days']):
        # Fluctuation score (higher = more erratic behavior)
        df['activity_fluctuation'] = np.abs(df['unique_proposal'] - df['unique_quotations']) / np.maximum(df['total_activity'], 1)
        
        # Detect agents with high activity variance
        df['high_variance_agent'] = (df['proposal_variance'] > df['proposal_variance'].median() * 2).astype(int)
        
        # Activity spikes
        df['proposal_spike'] = (df['unique_proposal'] > df['unique_proposals_last_7_days'] * 3).astype(int)
        df['quotation_spike'] = (df['unique_quotations'] > df['unique_quotations_last_7_days'] * 3).astype(int)
        
        # Potential data anomalies (extreme values - potential false positives)
        df['potential_anomaly'] = ((df['proposal_spike'] == 1) & (df['quotation_spike'] == 0)).astype(int)
...
for agent in unique_agents:
    ...
    if len(agent_data) > 0:
        ...
        nill_streak = 0
        max_nill_streak = 0
        nill_counts = (agent_data['new_policy_count'] == 0).sum()
        
        # Calculate pattern metrics
        for i in range(len(agent_data)):
            if agent_data.iloc[i]['new_policy_count'] == 0:
                nill_streak += 1
            else:
                nill_streak = 0
            max_nill_streak = max(max_nill_streak, nill_streak)
        
        # Detect alternating patterns (sale, no sale, sale, no sale)
        alternating_count = 0
        for i in range(1, len(agent_data)):
            prev_nill = agent_data.iloc[i-1]['new_policy_count'] == 0
            curr_nill = agent_data.iloc[i]['new_policy_count'] == 0
            if prev_nill != curr_nill:
                alternating_count += 1
        ...
        agent_patterns.append({
            ...
            'nill_counts': nill_counts,
            'alternating_pattern': alternating_count / max(1, len(agent_data) - 1),
            'record_count': len(agent_data)
        })

# Create pattern DataFrame
if agent_patterns:
    agent_patterns_df = pd.DataFrame(agent_patterns)
    agent_patterns_df['pattern_type'] = np.where(
        agent_patterns_df['alternating_pattern'] > 0.5, 'alternating',
        np.where(agent_patterns_df['max_nill_streak'] / agent_patterns_df['record_count'] > 0.5, 'streak', 'normal')
    )
    
    # Create indicator columns for pattern types
    agent_patterns_df['is_alternating_pattern'] = (agent_patterns_df['pattern_type'] == 'alternating').astype(int)
    agent_patterns_df['is_streak_pattern'] = (agent_patterns_df['pattern_type'] == 'streak').astype(int)
    agent_patterns_df['is_normal_pattern'] = (agent_patterns_df['pattern_type'] == 'normal').astype(int)
    
    # Merge patterns with profiles
    agent_profiles = pd.merge(agent_profiles, agent_patterns_df, on='agent_code', how='left')
...
for agent in tqdm(train_df['agent_code'].unique(), desc="Processing training agents"):
    ...
    for i in range(1, len(agent_data)):
        ...
        # New: Calculate fluctuation patterns
        if len(past_data) >= 3:
            recent_nill = []
            for idx in range(len(past_data) - min(3, len(past_data)), len(past_data)):
                recent_nill.append(past_data.iloc[idx]['new_policy_count'] == 0)
            
            # Check for alternating pattern in recent history
            alternating_recent = 0
            for j in range(1, len(recent_nill)):
                if recent_nill[j] != recent_nill[j-1]:
                    alternating_recent += 1
            
            hist_data['recent_alternating_pattern'] = alternating_recent / max(1, len(recent_nill) - 1)
    ...
    if len(agent_test_data) > 0:
        ...
        for i, test_row in agent_test_data.iterrows():
            ...
            if len(past_data) > 0:
                ...
                # New: Calculate fluctuation patterns
                if len(past_data) >= 3 and 'new_policy_count' in past_data.columns:
                    past_data_sorted = past_data.sort_values('year_month')
                    recent_nill = []
                    for idx in range(len(past_data_sorted) - min(3, len(past_data_sorted)), len(past_data_sorted)):
                        recent_nill.append(past_data_sorted.iloc[idx]['new_policy_count'] == 0)
                    ...
                    hist_data['recent_alternating_pattern'] = 0
...
non_numeric_cols = ['row_id', 'agent_code', 'exp_category', 'pattern_type'] + date_columns
for col in date_columns:
...
print("\nStep 4: Finding optimal threshold with cross-validation...")
for train_idx, val_idx in tscv.split(X_scaled):
    X_train, X_val = X_scaled[train_idx], X_scaled[val_idx]
    y_train, y_val = y.iloc[train_idx], y.iloc[val_idx]
    
    # Train CatBoost with strong bias for NILL prediction
    cat_model = cb.CatBoostClassifier(
        ...
        verbose=0,
        class_weights={0: 5.0, 1: 1.0}  # Much higher weight for NILL class (0)
    )
    cat_model.fit(X_train, y_train)
    
    # Get predictions
    val_probas = cat_model.predict_proba(X_val)[:, 1]
    
    # Store for threshold optimization
    cv_predictions.extend(val_probas)
    cv_true_values.extend(y_val)

# Convert to numpy arrays
cv_predictions = np.array(cv_predictions)
cv_true_values = np.array(cv_true_values)

# Find optimal base threshold
best_accuracy = 0
best_threshold = 0.5

# Try different base thresholds
for threshold in np.arange(0.3, 0.61, 0.01):
    cv_preds = (cv_predictions >= threshold).astype(int)
    accuracy = accuracy_score(cv_true_values, cv_preds)
    
    if accuracy > best_accuracy:
        best_accuracy = accuracy
        best_threshold = threshold

print(f"Cross-validation found optimal base threshold: {best_threshold:.4f} with accuracy: {best_accuracy:.4f}")

# Build final models with optimized parameters
print("\nStep 5: Training final models...")

# Stage 1: Train CatBoost with strong bias for NILL prediction
nill_model = cb.CatBoostClassifier(
    iterations=500,
    learning_rate=0.025,
    depth=7,
    l2_leaf_reg=3,
...
nill_model.fit(X_scaled, y)

# Stage 2: Train XGBoost with normal class balance
...
print("\nStep 6: Generating predictions with two-stage approach...")
...
nill_biased_proba = nill_model.predict_proba(X_test_scaled)[:, 1]
normal_proba = xgb_model.predict_proba(X_test_scaled)[:, 1]

# Two-stage prediction strategy
# 1. Start with nill-biased probabilities
test_proba = nill_biased_proba.copy()

# 2. For each test record, calculate a dynamic threshold
test_thresholds = np.zeros_like(test_proba)
for i in range(len(test_df)):
    base_threshold = best_threshold
    
    # Adjust for alternating pattern
    if 'is_alternating_pattern' in test_df.columns and test_df.iloc[i]['is_alternating_pattern'] == 1:
        # For alternating patterns, use recent history to predict next state
        if 'recent_alternating_pattern' in test_df.columns and test_df.iloc[i]['recent_alternating_pattern'] > 0.5:
            # Check if the most recent was a NILL
            if test_df.iloc[i]['hist_current_nill_streak'] > 0:
                # Last was NILL, predict non-NILL (decrease threshold)
                base_threshold -= 0.1
            else:
                # Last was non-NILL, predict NILL (increase threshold)
                base_threshold += 0.1
    
    # Adjust for high-risk agents
    if test_df.iloc[i]['high_risk_agent'] == 1:
        base_threshold += 0.05
    
    # Adjust for streak risk
    if test_df.iloc[i]['streak_risk'] == 1:
        base_threshold += 0.08
        
    # Adjust for new agents
    if test_df.iloc[i]['months_with_company'] <= 3:
        base_threshold += 0.03  # New agents more likely to go NILL
    
    # Adjust for agents with abnormally high activity
    if 'potential_anomaly' in test_df.columns and test_df.iloc[i]['potential_anomaly'] == 1:
        base_threshold += 0.06  # Likely a false positive
        
    # Adjust for declining momentum
    if test_df.iloc[i]['proposal_momentum'] < 0.7:
        base_threshold += 0.04
    
    # Enforce minimum and maximum thresholds
    test_thresholds[i] = np.clip(base_threshold, 0.3, 0.65)

# 3. Apply dynamic thresholds
ceiling_breaking_predictions = (test_proba >= test_thresholds).astype(int)

# 4. Override specific cases based on business rules
for i in range(len(test_df)):
    # Override rule 1: Agents with 3+ NILL streak almost always continue the streak
    if test_df.iloc[i]['hist_current_nill_streak'] >= 3:
        ceiling_breaking_predictions[i] = 0  # Force NILL prediction
        
    # Override rule 2: Agents with extremely high nill rate and recent NILLs
    if (test_df.iloc[i]['agent_nill_rate'] > 0.8 and 
        test_df.iloc[i]['hist_current_nill_streak'] >= 1):
        ceiling_breaking_predictions[i] = 0  # Force NILL prediction
        
    # Override rule 3: Handle borderline cases (nill_model and xgb_model strongly disagree)
    if abs(nill_biased_proba[i] - normal_proba[i]) > 0.3:
        # Strong disagreement - use the agent pattern to decide
        if 'is_streak_pattern' in test_df.columns and test_df.iloc[i]['is_streak_pattern'] == 1:
            # For streak patterns, trust the nill_model more
            ceiling_breaking_predictions[i] = (nill_biased_proba[i] >= test_thresholds[i]).astype(int)
        elif 'is_alternating_pattern' in test_df.columns and test_df.iloc[i]['is_alternating_pattern'] == 1:
            # For alternating patterns, use recent history to predict next state
            if test_df.iloc[i]['hist_current_nill_streak'] > 0:
                # Last was NILL, predict non-NILL
                ceiling_breaking_predictions[i] = 1
            else:
                # Last was non-NILL, predict NILL
                ceiling_breaking_predictions[i] = 0

# Create optimized submission
winning_submission = submission_template.copy()
winning_submission['target_column'] = ceiling_breaking_predictions
winning_submission_path = os.path.join(output_dir, 'ceiling_breaker_submission.csv')
winning_submission.to_csv(winning_submission_path, index=False)

print(f"\nCeiling-breaker submission file created: {winning_submission_path}")
print(f"Prediction counts: {pd.Series(ceiling_breaking_predictions).value_counts()}")
print(f"Prediction rate: {ceiling_breaking_predictions.sum()/len(ceiling_breaking_predictions):.2%} non-NILL")

# Save predictions for analysis
np.save(os.path.join(output_dir, 'nill_biased_probabilities.npy'), nill_biased_proba)
np.save(os.path.join(output_dir, 'normal_probabilities.npy'), normal_proba)
np.save(os.path.join(output_dir, 'dynamic_thresholds.npy'), test_thresholds)

# Save models for future use
model_path = os.path.join(output_dir, 'nill_biased_model.pkl')
joblib.dump((nill_model, scaler, final_features), model_path)

model_path = os.path.join(output_dir, 'normal_model.pkl')
joblib.dump((xgb_model, scaler, final_features), model_path)
...
print("\n" + "=" * 100)
print(f"FINAL CEILING-BREAKING MODEL completed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print(f"Total execution time: {elapsed_time:.2f} seconds ({elapsed_time/60:.2f} minutes)")
print(f"CEILING-BREAKER SUBMISSION: {winning_submission_path}")
print("=" * 100)